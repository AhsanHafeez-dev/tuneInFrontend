...
export default function ChannelPage() {
    ...
    const fetchChannelVideos = async (userId) => {
        try {
            const res = await fetch(`/api/v1/videos?userId=${userId}&page=1&limit=50`);
            const data = await res.json();
            if (res.ok) {
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (data.data.subscribed) {
                    setChannel(prev => ({ ...prev, subscribersCount: (prev.subscribersCount || 0) + 1 }));
                } else {
                    setChannel(prev => ({ ...prev, subscribersCount: (prev.subscribersCount || 0) - 1 }));
                }
            }
    ...
    if (loading) return <div style={{ padding: '2rem', textAlign: 'center' }}>Loading channel...</div>;
    if (!channel) return <div style={{ padding: '2rem', textAlign: 'center' }}>Channel not found</div>;

    return (
        <div className={styles.container}>
            <div className={styles.bannerWrapper}>
                {channel.coverImage ? (
                    <img src={channel.coverImage} alt="Cover" className={styles.coverImage} />
                ) : (
            ...
            <div className={styles.content}>
                                    ...
                                    >
                                        <div className={styles.playlistThumbWrapper}>
                                            {playlist.firstVideo?.thumbnail ? (
                                                <img src={playlist.firstVideo.thumbnail} alt={playlist.name} className={styles.thumbnail} />
                                            ) : (
                                                <div style={{ width: '100%', height: '100%', background: 'var(--secondary)' }}></div>
                                            )}
                                            <div className={styles.playlistOverlay}>
                                                <span style={{ color: 'white', fontWeight: 'bold' }}>{playlist.totalVideos}</span>
                                                <span style={{ color: 'white', fontSize: '0.8rem', display: 'block' }}>videos</span>
                                            </div>
                                        </div>
                                        <div className={styles.playlistInfo}>
                                            <h3 className={styles.playlistTitle}>{playlist.name}</h3>
                                            <span className={styles.playlistCount}>View full playlist</span>
                                        </div>
                                    </Link>
                                );
                            })
                ...
                {activeTab === 'tweets' && (
                    <div className={styles.tweetsContainer}>
                        {isOwner && (
                            <form onSubmit={handleCreateTweet} className={styles.createTweetForm}>
                                <textarea
                        ...
                        <div className={styles.tweetsList}>
                                ...
                                <div key={tweet._id} className={styles.tweetCard}>
                                    <div className={styles.tweetHeader}>
                                        ...
                                        <div className={styles.tweetMeta}>
                                            <span className={styles.tweetUser}>{channel.fullName}</span>
                            ...
                            {tweets.length === 0 && <p style={{ color: 'gray', textAlign: 'center' }}>No tweets yet.</p>}
                        </div>
                    </div>
                )}
            </div>
